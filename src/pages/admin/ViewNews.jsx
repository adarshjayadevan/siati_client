import {
  ...
  const AdminNews = () => {
    ...
    async function get_events() {
      // debugger
      setLoader(true);
      await adminAxios.get(`${import.meta.env.VITE_API_URL}/admin/news?pageNum=${activePage}&&perPage=${limit}`).then(res => {
  ...
  
          {
            loader ? <Box minH="70vh" display="flex" justifyContent="center" alignItems="center">
              ...
              <><div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Heading p={5}>Promotions</Heading>
                <Button p={5} background={'teal'} onClick={() => setAddModal(true)}>Add Promotion</Button>
              </div>
                  ...
                  <tbody>
                    {items.map((item, index) => (
                      <tr key={index} style={{ borderBottom: "1px solid #ddd" }}>
                        ...
                        <td style={{ padding: "8px", textAlign: "left" }}>{item.title.slice(0,50)}...</td>
