...
import EditAlbum from './EditAlbum';
  ...
  const AdminGallery = () => {
    ...
    async function get_events() {
      // debugger
      setLoader(true);
      await adminAxios.get(`${import.meta.env.VITE_API_URL}/admin/gallery?pageNum=${activePage}&&perPage=${limit}`).then(res => {
    ...
    async function deleteEvent(eventId, eventName) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          await adminAxios.delete(`${import.meta.env.VITE_API_URL}/admin/gallery/${eventId}`).then(res => {
            Swal.fire({
              title: "Deleted!",
    ...
    return (
      <Box minH="100vh" bg={useColorModeValue('gray.100', 'gray.900')}>
        ...
        <Box ml={{ base: 0, md: 60 }} p="4">
  
          {
            loader ? <Box minH="70vh" display="flex" justifyContent="center" alignItems="center">
              ...
              <><div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Heading p={5}>Album</Heading>
                <Button p={5} background={'teal'} onClick={() => setAddModal(true)}>Add Gallery Event</Button>
              </div>
                  ...
                  <tbody>
                    {items.map((item, index) => (
                      <tr key={index} style={{ borderBottom: "1px solid #ddd" }}>
                        ...
                        <td style={{ padding: "8px", textAlign: "left" }}>{item.title}</td>
                        ...
                        <td style={{ padding: "8px", textAlign: "center" }}>
                          <div style={{ display: 'flex' }}>
                            ...
                            <FaTrash title='Delete' onClick={() => deleteEvent(item._id, item.title)} style={{ cursor: "pointer", color: "red" }} />
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          <AddAlbum addModal={addModal} setAddModal={setAddModal} setEventFlag={setEventFlag} />
          <EditAlbum editModal={editModal} setEditModal={setEditModal} event={selectedEvent} setEventFlag={setEventFlag} />
        </Box>
      </Box>
    )
  }
